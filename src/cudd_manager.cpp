...
static std::string maybe_parenthesize(const BddFormula &formula, int min_precedence) {
    if (formula.precedence < min_precedence) {
        return "(" + formula.text + ")";
    }
    return formula.text;
}

static BddFormula bdd_ite_formula(
    DdManager *mgr,
    DdNode *node,
    std::unordered_map<DdNode *, BddFormula> &memo
) {
    constexpr int prec_or = 1;
    constexpr int prec_and = 2;
    constexpr int prec_not = 3;
    constexpr int prec_atom = 4;
    auto make_not = [&](const BddFormula &expr) {
        std::string inner = maybe_parenthesize(expr, prec_not);
        return BddFormula{"!" + inner, prec_not};
    };
    auto make_and = [&](const BddFormula &lhs, const BddFormula &rhs) {
        std::string left = maybe_parenthesize(lhs, prec_and);
        std::string right = maybe_parenthesize(rhs, prec_and);
        return BddFormula{left + " && " + right, prec_and};
    };
    auto make_or = [&](const BddFormula &lhs, const BddFormula &rhs) {
        std::string left = maybe_parenthesize(lhs, prec_or);
        std::string right = maybe_parenthesize(rhs, prec_or);
        return BddFormula{left + " || " + right, prec_or};
    };
    auto make_xor = [&](const BddFormula &lhs, const BddFormula &rhs) {
        return BddFormula{"xor(" + lhs.text + ", " + rhs.text + ")", prec_atom};
    };
    ...
    if (Cudd_IsConstant(regular)) {
        ...
        return BddFormula{is_one ? "TRUE" : "FALSE", prec_atom};
    }
    if (complemented) {
    ...
    BddFormula var{"x" + std::to_string(index), prec_atom};
    ...
    if (t_reg == e_reg && t_comp != e_comp) {
        if (t_comp && !e_comp) {
            BddFormula sub = bdd_ite_formula(mgr, e, memo);
            formula = make_xor(var, sub);
        } else {
            BddFormula sub = bdd_ite_formula(mgr, t, memo);
            formula = make_not(make_xor(var, sub));
        }
    } else if (t_const && e_const) {
        if (t_value == e_value) {
            formula = BddFormula{t_value ? "TRUE" : "FALSE", prec_atom};
        } else if (t_value) {
            formula = var;
        } else {
        ...
        BddFormula e_formula = bdd_ite_formula(mgr, e, memo);
        formula = make_or(var, e_formula);
    } else if (t_const && !t_value) {
        BddFormula e_formula = bdd_ite_formula(mgr, e, memo);
        formula = make_and(make_not(var), e_formula);
    } else if (e_const && !e_value) {
        BddFormula t_formula = bdd_ite_formula(mgr, t, memo);
...
extern "C" SEXP c_cudd_reduce_heap(SEXP mgr_ptr, SEXP heuristic, SEXP minsize) {
    Cudd *mgr = cudd_manager_from_ptr(mgr_ptr);
    int minsize_value = Rf_asInteger(minsize);
    if (minsize_value == NA_INTEGER || minsize_value < 0) {
        Rf_error("'minsize' must be a non-negative integer.");
    }
    if (Rf_isNull(heuristic)) {
        mgr->ReduceHeap(CUDD_REORDER_SIFT, minsize_value);
    } else {
        int value = Rf_asInteger(heuristic);
        if (value == NA_INTEGER) {
            Rf_error("'heuristic' must be a valid integer.");
        }
        mgr->ReduceHeap(static_cast<Cudd_ReorderingType>(value), minsize_value);
    }
    return R_NilValue;
}
...
extern "C" SEXP c_cudd_bdd_ite_formula(SEXP bdd_ptr) {
    BDD *bdd = bdd_from_ptr(bdd_ptr);
    std::unordered_map<DdNode *, BddFormula> memo;
    BddFormula formula = bdd_ite_formula(bdd->manager(), bdd->getNode(), memo);
    return Rf_mkString(formula.text.c_str());
}
